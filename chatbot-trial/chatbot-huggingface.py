...
def setup_chain():
    ...
    # Use a HuggingFaceHub LLM (e.g., google/flan-t5-base)
    llm = HuggingFaceHub(
...
if st.session_state.chat_history:
    ...
    # Display newest Q&A at the top
    for i in range(len(history) - 2, -1, -2)[::-1]:
        speaker_q, text_q = history[i], history[i+1]
        # Question
        if speaker_q[0] == "You":
            st.markdown(
                f"""
                <div style='background:#1565c0;padding:10px 14px;border-radius:8px;margin-bottom:2px'>
                    <strong style='color:#fff;'>You:</strong> <span style='color:#e3f2fd'>{speaker_q[1]}</span>
                </div>
                """,
        ...
        if speaker_q[0] == "Bot":
            st.markdown(
                ...
                <div style='background:#fffde7;padding:10px 14px;border-radius:8px;margin-bottom:8px'>
                    <strong style='color:#f9a825;'>Bot:</strong> <span style='color:#6d4c41'>{speaker_q[1]}</span>
                </div>
                """,
                unsafe_allow_html=True
            )
        if speaker_q[0] == "You" and speaker_q != history[-2]:
            # Show the answer after the question
            st.markdown(
                f"""
                <div style='background:#fffde7;padding:10px 14px;border-radius:8px;margin-bottom:8px'>
                    <strong style='color:#f9a825;'>Bot:</strong> <span style='color:#6d4c41'>{history[i+1][1]}</span>
                </div>
                """,
                unsafe_allow_html=True
            )